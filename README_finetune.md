...
T3Cond_embeds
Generated by T3CondEnc:
The T3CondEnc class is responsible for handling all non-text conditioning, such as:
Speaker embeddings: Processed through a linear layer (spkr_enc).
Emotion scalar: Processed through an optional fully connected layer (emotion_adv_fc).
Prompt speech embeddings: Resampled using a Perceiver module (self.perceiver).
Optional CLAP embeddings: Included if provided.
These components are concatenated into a short prefix sequence (T3Cond_embeds) that conditions the model.

speech_embeds
Generated by the Transformer:
The T3 class uses a Transformer-based architecture (e.g., LLaMA or GPT-2) to process the token stream.
The speech_embeds are part of the token stream and represent the acoustic features of speech tokens.
Key Differences
T3Cond_embeds:
Generated by T3CondEnc using speaker embeddings, CLAP embeddings, prompt speech embeddings, and emotion scalars.
Acts as a conditioning prefix for the token stream.
speech_embeds:
Generated by the Transformer as part of the token stream.
Represents the raw acoustic features of speech tokens